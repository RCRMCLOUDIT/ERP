...
public class AutoInvoiceRptHelper extends ERPReport {
	...
	private final int CURRENCY_COL  = 5;
	//private final int HRTERM_COL 	= 6;
	private final int HRTOTA_COL 	= 7;
	private final int HRTTAX_COL 	= 8;
	private final int HRDDAT_COL 	= 9;
	//private final int HRSTAT_COL 	= 10;
	private final int PPTEXA_COL 	= 11;
	private final int CMNAME_COL	= 12;
	
	private final int CMADD1_COL 	= 13;
	private final int CMADD2_COL 	= 14;
	private final int CMSTAT_COL	= 15;
	private final int CMCITY_COL	= 16;
	private final int CMZIP_COL     = 17;
	private final int CMPROV_COL    = 18;
	private final int CMCOUNTRY_COL = 19;
    ...
    public AutoInvoiceRptHelper(Rectangle pageSize) {
        super(pageSize);
		setMargins(LEFT, RIGHT, TOP, BOTTOM);
    }
    public AutoInvoiceRptHelper(
        Rectangle pageSize,
        float marginLeft,
        float marginRight,
        float marginTop,
	...
	public AutoInvoiceRptHelper(String fname) throws DocumentException
	{
		super(fname);
		setMargins(LEFT, RIGHT, TOP, BOTTOM);
	}
	
	public AutoInvoiceRptHelper(HttpServletRequest request) throws DocumentException {
        super(request);
        if( request.getParameter("maintenanceAction") != null && request.getParameter("maintenanceAction").equals("REPRINT")){
	        invoiceOnly = true;
        }
		setMargins(LEFT, RIGHT, TOP, BOTTOM);
    }

	private void addDetails(InvoiceDataTable table, int detailRow, int invoiceRow ) throws BadElementException,
		...
		int[] aligns = {Element.ALIGN_CENTER,
					   ...
					   Element.ALIGN_RIGHT,
					   Element.ALIGN_RIGHT,};
		table.printTblCells(value, aligns, 15f);
	}
...
/*
	contactInfo[0] = getTrimedStr(dataBean.valueAtColumnRowResult(CCNAME_COL,row,headerInfoCursor));
	contactInfo[1] = getTrimedStr(dataBean.valueAtColumnRowResult(CMNAME_COL,row,headerInfoCursor));
	contactInfo[2] = getTrimedStr(dataBean.valueAtColumnRowResult(CMADD1_COL,row,headerInfoCursor));
	contactInfo[3] = getTrimedStr(dataBean.valueAtColumnRowResult(CMADD2_COL,row,headerInfoCursor));
*/
	...
	public void addSummary(String companyName, int invoices, PdfWriter writer,String[] coInfo, String preComm)throws BadElementException,DocumentException, Exception {
		...
		{
			int invoNum = ((BigDecimal)dataBean.valueAtColumnRowResult(HRINVO_COL, i, headerInfoCursor)).intValue();
			...
			String invDate = Format.date_Str((String)dataBean.valueAtColumnRowResult(HRDATE_COL,i,headerInfoCursor));
			String dueDate = Format.date_Str((String)dataBean.valueAtColumnRowResult(HRDDAT_COL,i,headerInfoCursor));
			String custName = (String)dataBean.valueAtColumnRowResult(CMNAME_COL,i,headerInfoCursor);
			String payTerm = (String)dataBean.valueAtColumnRowResult(PPTEXA_COL,i,headerInfoCursor);
	...
	private void addTblHeader(InvoiceDataTable table, int row) throws BadElementException, DocumentException, Exception 
	{
		...
		ReportCell[] headerText = {new ReportCell("", 1, 1),
								...
								new ReportCell("", 1, 1, Element.ALIGN_LEFT),
								new ReportCell("", 1, 1),
								new ReportCell("", 1, 1),
	...
	public void buildPDFInvoice(String companyName)throws BadElementException, DocumentException, Exception 
	{
		...
		comments = "";//new String[numOfUserComments];
...
private String getStandardComments()throws BadElementException, DocumentException, Exception
{
	...
	/*
	String[] standardComments = new String[7];
