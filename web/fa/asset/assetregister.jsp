...
String search = java.net.URLDecoder.decode(request.getParameter("search"));
    ...
    <TR class="pcrinfo">
	    ...
	    <TD colspan="3">
			<INPUT type="hidden" name="manuId" id="manuId" value="<%=manuId%>">
      		<INPUT type="text" name="manuName" id="manuName" size="30" maxlength="40" value="<%=manuName%>" onchange="myForm.manuId.value='';" onblur="myForm.manuId.value='';" onkeypress='if(isEnterPressed()) openEntDialog("MNF", "", "myForm", "manuId", "manuName", "<%=contextRoot%>");'>
			<INPUT type="button" name="searchMan" value=" v " onclick='openEntDialog("MNF", "", "myForm", "manuId", "manuName", "<%=contextRoot%>");'></TD>
    </TR>   
    <TR class="pcrinfo1">
        <TD class="label">&nbsp;<%=rb.getString("P12120009")%></TD>
	    <TD colspan="3">
			<INPUT type="hidden" name="vendId" id="vendId" value="<%=vendId%>">
      		<INPUT type="text" name="vendName" id="vendName" size="30" maxlength="40" value="<%=vendName%>" onchange="myForm.vendId.value='';" onblur="myForm.vendId.value='';" onkeypress='if(isEnterPressed()) openEntDialog("VEN", "", "myForm", "vendId", "vendName", "<%=contextRoot%>");'>
...
<% }else { %>
			<INPUT type="text" name="acqDate" size="10" maxlength="10" value="<%=acqDate%>" onkeyup="addSlash(myForm.acqDate)" onchange="formatDate(myForm.acqDate)" onkeypress="OnlyDigits();addSlash(myForm.acqDate)" onkeydown="return checkArrows(this, event)">
			<IMG border="0" onclick="popUpCalendar(this, myForm.acqDate, 'mm/dd/yyyy', -1, -1);" alt="Calendar" src="../erp/ERP_COMMON/images/popcalendar/calendar.gif">
<% } %>			
		</TD>
...
<% if(status.equals("RG")) { %>		
				...
				<TR class="pcrinfo1">
					...
					<TD><INPUT type="hidden" name="level1" value="<%=glDepAcct%>"> 
					    <INPUT type="text" size="70" name="levelText1" value="<%=Format.encodeHTML(levelText1)%>" class="textfieldDisable" readOnly>
					</TD>
    			</TR>
				<TR class="pcrinfo">
      	 			<TD class="label"><span class="textRed">*</span><%=rb.getString("P12120024")%></TD>
					<TD><INPUT type="hidden" name="level2" value="<%=glExpAcct%>"> 
					    <INPUT type="text" size="70" name="levelText2" value="<%=Format.encodeHTML(levelText2)%>" class="textfieldDisable" readOnly>
					</TD>
    			</TR>
<% }else { %>
				<TR class="pcrinfo1">
